...
var Surku = function (user_config){
	...
	this.generateTestCase=function(input, chunkSize){
		chunkSize = (chunkSize === undefined)? this.config.chunkSize : chunkSize;
		var testCase
		this.config=self.config
		this.storage=self.storage
		this.debugPrint= log/*function(message,level){
		...
		if(input instanceof Buffer){
			input=new Buffer((mutate.call(this,input.toString('binary'), chunkSize)),'binary')
			return input
		}
		else if(input instanceof String || typeof(input)=='string'){
			return mutate.call(this,input, chunkSize)
		}
		else{
	...
	*/
	...
	//
	//
	function mutate(input, chunkSize){	
		chunkSize = (chunkSize === undefined)? this.config.chunkSize : chunkSize;
		if(input.length!=0){
			...
			while(mutations--){
					...
					if(input.length>chunkSize)
						index=this.rint(input.length-chunkSize)
					var mutator=this.ra(this.mutators.mutators)
					var chunk=input.substring(index,index+chunkSize)
					...
					if(result === false){
						...
						input=input.substring(0,index)+result+input.substring(index+chunkSize)			
					}
			}
			return input
		}
