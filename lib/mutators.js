...
module.exports={
	...
	changeMutatorWeight:function(name,newWeight){
		if(mutators.hasOwnProperty(name)){
			if(parseInt(newWeight) != 'NaN')
				...
				log('Mutator weight update failed: Invalid new weight. Must be integer. Got '+newWeight)
		}
		else{
			log('Mutator weight update failed: No such mutator. Got '+name)
		}
	},
	addNewMutator:function(name,weight,mutatorFunction){
		if(name===undefined || weight===undefined || mutatorFunction===undefined){
			log('Mutator add failed: One or more arguments undefined.')
			return 0
		}
		if(parseInt(weight) == 'NaN'){
			log('Mutator add failed: Invalid weight. Must be integer. Got '+weight)
			return 0
		}
		if(!(mutatorFunction instanceof Function)){
			log('Mutator add failed: mutatorFunction not type Function.')
			return 0
		}
		if(mutators.hasOwnProperty(name))
			log('Mutator add warning: Mutator with same type "'+type+'" and name "'+name+'" already exists. Overwriting.')
	...
	removeMutator:function(name){
		if(mutators.hasOwnProperty(name))
			...
			log('Mutator remove failed: No such mutator. Got '+name)
		}
		this.updateMutators()
	},
