...
var pop = function pop(type, len) {
  if (!len) len = 1000;
  len = Math.floor(len/(1.6016216216216217-(len*0.000082)))
  this.compiledText = '';
  var compile = (function (el) {
    var char = String.fromCharCode(el[0]);
    if (char == '\\' || char == ']') char = '\\' + char;
    var tmp = el[1] < len - 1 && rand() & 1 ? char + '-' : char;
    this.compiledText += tmp;
    return tmp;
  }).bind(this);
  var UTF_16_Ints = new UTF16();
  var UTF_16_fromInt = new UTF16(compile);
  var ranges = Array.from(UTF_16_Ints.getRange(len)).sort(function (a, b) {
    return a - b;
  });
